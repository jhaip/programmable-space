...
void publishValueMessages(String val1, String val2, String val3, String val4)
{
  char str[600];
  sprintf(str, "{\"claim\":[\"Photon%s read \\\"%s\\\" on sensor %i\", \"Photon%s read \\\"%s\\\" on sensor %i\", \"Photon%s read \\\"%s\\\" on sensor %i\", \"Photon%s read \\\"%s\\\" on sensor %i\"], \"retract\":\"$ $ Photon%s read $ on sensor $\"}",
...
void loop()
{
  ...
  Serial.printlnf("%s %s %s %s", val_a.c_str(), val_b.c_str(), val_c.c_str(), val_d.c_str());
